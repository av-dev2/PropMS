...
def validate_property_increment_settings(doc, method=None):
    ...
    for row in rules:
        ...
        if dedupe_key in seen_rules:
            frappe.throw(_("Duplicate active rule found for Lease Item {0} on {1}.").format(lease_item, dedupe_key[1]))
...
def _process_property(property_name):
    ...
    for lease_meta in leases:
        try:
            lease = frappe.get_doc("Lease", lease_meta.name)
            lease_changed = False
            ...
            for rule in rules:
                ...
                item_rows = [row for row in (lease.get("lease_item") or []) if row.lease_item == lease_item_code]
                if not item_rows:
                    continue

                current_rows = _get_effective_rows(lease, item_rows)
                existing_dates = {row["effective_date"] for row in current_rows}
                candidate_date = getdate(effective_from)
                    ...
                    if candidate_date >= today and candidate_date not in existing_dates:
                        ...
                        if prev_row:
                            ...
                            lease.append(
                                "lease_item",
                                {
                                    "lease_item": prev_row["row"].lease_item,
                                    "frequency": prev_row["row"].frequency,
                                    "amount": new_amount,
                                    "currency_code": prev_row["row"].currency_code,
                                    "charge_basis": prev_row["row"].charge_basis,
                                    "charge_rate": prev_row["row"].charge_rate,
                                    "witholding_tax": prev_row["row"].witholding_tax,
                                    "paid_by": prev_row["row"].paid_by,
                                    "invoice_item_group": prev_row["row"].invoice_item_group,
                                    "document_type": prev_row["row"].document_type,
